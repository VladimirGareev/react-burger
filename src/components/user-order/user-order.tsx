...
export const UserOrder:FunctionComponent<TUserOrder> = ({ order }) => {
  ...
  if (selectedIngredients[0]) {
    selectedIngredients.map((ingredient, index1) => {
      ...
      (ingredient as TIngredient).count = countNumber.length;
  ...
  const price = (selectedIngredients as TIngredient[]).reduce(
          (sum, ingredient) => sum + ingredient.price * (ingredient.count as number),
          0
        );
                ...
                return (
                  ...
                  >
                    <img
                      ...
                      alt={ingredient?.name}
                    />
                    {index < 5 && (ingredient as TIngredient).count as number > 1 && (
                      <Counter
                        count={(ingredient as TIngredient).count as number}
                        size="small"
