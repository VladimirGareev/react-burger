...
import { FormattedDate, Counter, CurrencyIcon } from "@ya.praktikum/react-developer-burger-ui-components"
...
if(selectedIngredients[0]) {selectedIngredients.map((ingredient, index1)=>{
   ...
   const price =
    ...
    <div className={styles.order__container}>
        ...
        <p className={`text text_type_main-medium ml-6 mb-6 mr-6 ${styles.order__name}`}>{`${order.name}`}</p>
        <div className={styles.ingredients__container}>
          <ul className={`${styles.images} ml-6`}>
            {selectedIngredients[0] && selectedIngredients.map((ingredient, index) => {
              if (index < 6) {
                return (
                  <li
