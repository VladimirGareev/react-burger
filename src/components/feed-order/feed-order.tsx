...
import { TFeedOrder } from "../../types/order";

export const FeedOrder:FunctionComponent<TFeedOrder> = ({ order }) => {
  ...
  if (selectedIngredients[0]) {
    selectedIngredients.map((ingredient, index1) => {
      ...
      if (ingredient) {ingredient.count = countNumber.length};
        ...
        <ul className={`${styles.images} ml-6`}>
          {selectedIngredients[0] &&
            selectedIngredients.map((ingredient, index) => {
              if (index < 6&&ingredient&&ingredient.count) {
                return (
                  <li
